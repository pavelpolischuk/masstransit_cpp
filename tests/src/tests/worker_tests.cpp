...
namespace masstransit_cpp_tests
{
	...
	TEST_CASE("worker_tests", "[queue_worker]")
	{
		SECTION("worker_do_task_on_other_thread")
		{
			...
			q.enqueue([&thread_id]{ thread_id = this_thread::get_id(); }).get();
			
			this_thread::sleep_for(chrono::milliseconds(50));
			q.stop();
			
			REQUIRE(thread_id != this_thread_id);
		}
		...
		SECTION("worker_thread_do_tasks_on_other_thread")
		{
			...
			const auto this_thread_id = this_thread::get_id();
			auto thread_id_1(this_thread_id);
			auto thread_id_2(this_thread_id);

			mutex m;
			m.lock();
			auto task_1 = th.enqueue([&thread_id_1, &m]
			{
				unique_lock<mutex> lock(m);
				thread_id_1 = this_thread::get_id();
			});
			
			auto task_2 = th.enqueue([&thread_id_2, &m]
			{
				unique_lock<mutex> lock(m);
			...
			REQUIRE(thread_id_1 != this_thread_id);
			REQUIRE(thread_id_2 != this_thread_id);
